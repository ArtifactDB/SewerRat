...
func mockDirectory(path string) error {
    ...
    err = os.WriteFile(filepath.Join(path, "metadata.json"), []byte(`{ "foo": "Aaron had a little lamb", "bar": { "breed": [ "merino", "border leicester" ], "type": "lamb", "number": 1 } }`), 0644)
    if err != nil {
    ...
    err = os.WriteFile(filepath.Join(sub, "metadata.json"), []byte(`{ "characters": [ { "first": "Akari", "last": "Akaza" }, { "first": "Hoshino", "last": "Kyouko" }], "anime": "Yuri Yuru", "id": 10495 }`), 0644)
    if err != nil {
        return fmt.Errorf("failed to mock a metadata file; %w", err)
    }

    err = os.WriteFile(filepath.Join(sub, "other.json"), []byte(`{ "name": "chicken tikka masala", "recipe": [ "chicken", "garlic", "rice", "spices" ], "variants": [ "lamb", "fish", "cheese" ] }`), 0644)
    if err != nil {
        return fmt.Errorf("failed to mock a metadata file; %w", err)
    }
    ...
    err = os.WriteFile(filepath.Join(sub2, "other.json"), []byte(`{ "favorites": [ "Yuru Camp", "Non non biyori" ] }`), 0644)
    if err != nil {
        return fmt.Errorf("failed to mock a metadata file; %w", err)
    }
